...
void loop() {
  ...
  while(0){
    ...
    // SerialUSB.print("total_time: "); SerialUSB.println(finish_time - start_time);
  }
}
...
void send_serial(){
  // unsigned long starttime = micros(); //uncomment for  time testing
   if(send_data and Serial){
       ...
       SerialUSB.println(sensed_M1_v, 3);
       SerialUSB.println(sensed_M2_v, 3);
       SerialUSB.println(sensed_MT_v, 3);
       SerialUSB.println(sensed_drive_angle * 180.0 / PI, 3);
       // SerialUSB.println(hamr_loc.theta * 180.0 / PI, 2);

       SerialUSB.println(desired_h_xdot, 3);
       SerialUSB.println(desired_h_ydot, 3);
       SerialUSB.println(sensed_drive_angle * 180.0 / PI, 3);

       SerialUSB.println(desired_h_xdot, 3);
       SerialUSB.println(desired_h_ydot, 3);
       SerialUSB.println(desired_h_rdot, 3);

       // SerialUSB.println(desired_dd_v);
       // SerialUSB.println(desired_dd_r);
   }
  // unsigned long finishtime = micros(); //uncomment for  time testing
  // SerialUSB.print("total_time: "); SerialUSB.println(finishtime - starttime); //uncomment for  time testing
}
...
void init_actuators(){
  ...
  digitalWrite(M1_DIR_PIN, LOW); // LOW is forwards
  digitalWrite(M2_DIR_PIN, HIGH); // HIGH is forwards
  digitalWrite(MT_DIR_PIN, HIGH); // HIGH is CLOCKWISE (use low for default)
...
//  SerialUSB.print("M1_PWM_PIN: "); SerialUSB.println(M1_PWM_PIN);
//  SerialUSB.print("M2_PWM_PIN: "); SerialUSB.println(M2_PWM_PIN);
  SerialUSB.print("MT_PWM_PIN: "); SerialUSB.println(MT_PWM_PIN);
  SerialUSB.print("pwm: "); SerialUSB.println(pwm);
  delay(50);
}
...
void request_decoder_count(char slave_addr){
  ...
  if(bytes_available == 4)
  {
    ...
    SerialUSB.print("count0: "); SerialUSB.print(decoder_count_M1);
    SerialUSB.print(" count1: "); SerialUSB.println(decoder_count_M2);
    // SerialUSB.print(" count2: "); SerialUSB.println(decoder_count_MT);

  }
  else
  {
    SerialUSB.println("I2C error. Bytes Received: "); SerialUSB.println(bytes_available);
...
void test_ADA(){
  ...
  SerialUSB.print("pwm_M1: "); SerialUSB.println(pwm_M1);
  SerialUSB.print("pwm_M2: "); SerialUSB.println(pwm_M2);
  SerialUSB.print("pwm_MT: "); SerialUSB.println(pwm_MT);
  SerialUSB.print("pwm: "); SerialUSB.println(pwm);
  delay(10);
}
