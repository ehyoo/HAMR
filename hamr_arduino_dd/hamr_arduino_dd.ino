...
float speed_req_ddrive = 0; // desired velocity for Diff Drive (m/s)
...
void loop() {
  ...
  if((millis()-lastMilli) >= LOOPTIME) {
    ...
    if (millis() - startMilli >= 0) { // conditional used to delay start of control loop for testing
      ...
      float m1 = speed_req_ddrive - speed_req_turret * 0.19812 * PI / 2.0;
      float m2 = speed_req_ddrive + speed_req_turret * 0.19812 * PI / 2.0;

      // compute PWM value
      update_pid(&pid_vars_M1, &PWM_M1, m1, speed_act_M1, t_elapsed);
      update_pid(&pid_vars_M2, &PWM_M2, m2, speed_act_M2, t_elapsed);
